...
import { SITES, getMonthYear, getToday } from "../lib/data";
import { db } from "../lib/db";
import type { FocusItem, Ticket } from "../lib/db/schema";
import { costs, digestItems, digestSummary, focusItems, tickets } from "../lib/db/schema";
...
export default function Overview({ loaderData }: Route.ComponentProps) {
  const { allTickets, digest, items, focus, totalVisitors, totalSpend, openTickets, greeting, dayName, dateStr } =
    loaderData;
  ...
  return (
    <>
      ...
      <div className="section-h">
        <h2>
          The three <em>missions</em>
        </h2>
        <span className="more">All properties →</span>
      </div>
      <div className="missions">
        {SITES.map((s) => (
          <MissionCard key={s.id} site={s} />
        ))}
      </div>
      ...
      <div className="grid-2">
        ...
        <div className="card digest-card">
          <div className="digest-head">
            <h3>
              Today, in <em>machines that build software</em>
            </h3>
            <span className="digest-when">
              {new Date().toLocaleTimeString("en-GB", { hour: "2-digit", minute: "2-digit" })} CET
            </span>
          </div>
          {digest ? (
            <>
            ...
            <p className="digest-para" style={{ color: "var(--mute)" }}>
            ...
            <div className="compose-row">
              <div className="seg">
                {(["alvarny", "studios", "labs"] as const).map((s) => (
                  <button key={s} className={`${s}${draftSite === s ? " on" : ""}`} onClick={() => setDraftSite(s)}>
                    {s}
                  </button>
                ))}
              </div>
